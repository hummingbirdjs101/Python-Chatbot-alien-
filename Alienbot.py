...
class AlienBot:
  ...
  def __init__(self):
    self.alienbabble = {'describe_planet_intent': r'.*\s*your planet.*',
                        'answer_why_intent': r'why\sare\syou.*',
                        'cubed_intent': r'.*cube.*(\d+)'
                            }

  # Define .greet() below:
  def greet(self):
    self.name = input("Welcome, what's your name?\n>")

    will_help = input(f"Hi {self.name}, I'm Martian 2106. I'm not from this planet. Will you help me learn about your planet?\n>")
  ...
  def match_reply(self, reply):
    for key, value in (self.alienbabble).items():
      ...
      if found_match and intent == 'describe_planet_intent':
        ...
        #self.cubed_intent(found_match.group()[0])
      else:
  ...
  def cubed_intent(self, number):
    ...
    return (f'The cube of {number} is {cubed_number}. Isnt that cool?')
